...
import { queueUpdate, flushSyncQueue } from '../utils/syncQueue';
...
export const useApplicationData = () => {
    ...
    // Ref mirror lets callbacks read fresh history without depending on it
    // (a state dep would retrigger the Firestore effect on every snapshot).
    const bodyWeightHistoryRef = useRef(bodyWeightHistory);
    useEffect(() => {
        bodyWeightHistoryRef.current = bodyWeightHistory;
    }, [bodyWeightHistory]);
    ...
    const handleUpdateAndSave = useCallback((updates) => {
        if (db && customId) {
            ...
            queueUpdate(userDocRef, updates);
        }
    }, [db, customId]);
    ...
    const handleBodyWeightChange = useCallback((newWeight, save = false) => {
        if (save) {
            ...
            if (!isNaN(weightValue) && weightValue > 0) {
                ...
                // Compute next history outside the state updater: updaters must
                // stay pure (no toasts / sync writes inside render-phase work).
                const newHistory = [...bodyWeightHistoryRef.current, newEntry];
    ...
    const handleResetMeso = async () => {
        if (db && customId && Object.keys(allLogs).length > 0) {
            ...
            await flushSyncQueue();
            queueUpdate(doc(db, 'workoutLogs', customId), { logs: {}, skippedDays: {} });
        }
    };
    ...
    const handleRestoreLogs = useCallback((csvData) => {
        ...
        try {
            ...
            openModal(
                <div>
                    <p className="ee-eyebrow text-coral">Restore data</p>
                    <h2 className="mt-2 font-display text-xl font-bold text-bone">Confirm restore</h2>
                    ...
                    <div className="mt-6 flex justify-end gap-2">
                        ...
                        <button onClick={() => {
                            ...
                            closeModal();
                        }} className="ee-danger">Confirm &amp; restore</button>
                    </div>
                </div>
            );
         ...
         openModal(
            <div>
                <div className="text-center">
                    <span className="mx-auto flex h-14 w-14 items-center justify-center rounded-full border border-amber/40 bg-amber/10 font-display text-lg font-bold text-amber">0:00</span>
                    <h2 className="mt-4 font-display text-xl font-bold text-bone">Time's up</h2>
                    <p className="mt-2 text-sm text-mute">Rest complete. Attack the next set.</p>
                    <div className="mt-6 flex justify-center">
                        <button onClick={closeModal} className="ee-primary px-8">Let's go</button>
                    </div>
                </div>
            </div>
        );
