...
export const RenameWorkoutModal = ({ oldName, onSave, onClose }) => {
    ...
    return (
        <div>
            <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-4">Rename Workout</h2>
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">Enter a new name for "{oldName}".</p>
            <input 
                type="text"
                value={newName}
                onChange={(e) => setNewName(e.target.value)}
                className="w-full p-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-white rounded-md border-gray-300 dark:border-gray-600"
                autoFocus
            />
