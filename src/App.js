...
const MainView = ({ onSessionSelect, onNavChange, completedDays }) => {
    ...
    return (<div className="p-4 md:p-6"><div className="flex flex-col sm:flex-row justify-between items-start mb-6 gap-4"><div className="flex items-center"><Dumbbell className="text-blue-500 dark:text-blue-400 mr-3" size={36} /><div><h1 className="text-3xl font-bold dark:text-white">{programInfo.name}</h1><p className="text-lg text-gray-600 dark:text-gray-400"></p></div></div><div className="flex gap-2 w-full sm:w-auto"><button onClick={() => onNavChange('dashboard')} className="flex-1 sm:flex-none justify-center flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg shadow-md hover:bg-blue-700 transition-colors"><BarChart2 size={16} /> <span className="sm:hidden">Dashboard</span></button><button onClick={() => onNavChange('settings')} className="p-3 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-lg shadow-md hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors"><Settings size={16} /></button></div></div><div className="space-y-4 pb-24">{weeks.map(week => (<WeekView key={week} week={week} completedDays={completedDays} onSessionSelect={onSessionSelect} firstIncompleteWeek={firstIncompleteWeek} />))}</div></div>);
};
